...
- Daily maintenance purges RSVP data after the event retention deadline, removes expired registrant payloads, reports stuck fulfillment, overdue privacy work, or feedback that has not been addressed within 24 hours, and records a server-only durable success/failure heartbeat.
...
The authenticated `/api/health/ready` endpoint is the machine-readable launch gate. Draft legal documents, missing external approval, stale or failed maintenance, or product feedback left actionable for more than 24 hours make it return HTTP 503.
