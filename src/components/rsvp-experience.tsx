...
function HeroSection({ t }: { t: WeddingTemplateCopy }) {
  return (
      ...
      >
        <div className="relative w-full max-w-[760px]">
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-white/70 bg-white/50 p-3 shadow-[0_30px_90px_rgba(65,42,36,0.18)] backdrop-blur-xl sm:p-4">
            <SampleInvitationTemplate imageAreaLabel={t.imageAreaLabel} />
          </div>
        </div>
        ...
        >
          ...
          <motion.div animate={{ y: [0, 6, 0] }} transition={{ duration: 1.4, repeat: Infinity, ease: "easeInOut" }}>
            <ChevronDown className="h-5 w-5 text-[#6f3032]/60" />
          </motion.div>
        </motion.a>
      </motion.div>
    </section>
    ...
    <section className="px-5 py-14 sm:px-8 sm:py-20 lg:px-12">
      <div className="mx-auto max-w-3xl">
        <ScrollReveal className="mb-10 text-center sm:mb-14">
          <h2 className="font-display text-4xl text-[#1f1a17] sm:text-5xl">{t.scheduleTitle}</h2>
...
function TimelineItem({ event }: { event: WeddingTemplateCopy["events"][number] }) {
  return (
    <ScrollReveal className="flex gap-4 sm:gap-7">
      ...
      >
        <h3 className="font-display text-2xl text-[#6f3032] sm:text-3xl">{event.title}</h3>
        <p className="mt-1 text-sm text-[#1f1a17]/55">{event.description}</p>
    ...
    <section className="px-5 py-16 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-5xl">
        <ScrollReveal className="text-center">
          ...
          <h2 className="font-display text-4xl text-[#1f1a17] sm:text-5xl">{t.venueTitle}</h2>
          <p className="mt-5 font-display text-2xl text-[#1f1a17]">{t.venueName}</p>
          <p className="mx-auto mt-2 max-w-sm text-sm leading-7 text-[#1f1a17]/58">{t.hallInfo}</p>
          <div className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[#6f3032] px-6 text-sm font-semibold text-white shadow-[0_18px_50px_rgba(111,48,50,0.22)] sm:w-auto">
      ...
      <AnimatePresence mode="wait">
        {isComplete ? (
          ...
          >
            ...
            <h2 className="font-display text-4xl text-[#1f1a17] sm:text-5xl">{t.thanksTitle}</h2>
          ...
          >
            <div className="mb-8 text-center">
              ...
              <h2 className="font-display text-4xl text-[#1f1a17] sm:text-5xl">{t.replyTitle}</h2>
              <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-[#1f1a17]/58">{t.replySubtitle}</p>
                ...
                >
                  ...
                  <div>
                    <div className="mb-4 flex items-center justify-between gap-4">
                      <div>
                        <h3 className="font-display text-2xl text-[#1f1a17]">{t.guestsTitle}</h3>
                      ...
                      <button
                        ...
                        className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-full border border-[#6f3032]/20 bg-white/55 px-4 text-sm font-semibold text-[#6f3032] shadow-sm transition hover:-translate-y-0.5 hover:bg-white/75 disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:translate-y-0"
                      >
                    ...
                    <AnimatePresence>
                      {hasDuplicateGuests ? (
                        <motion.p
                          className="mt-4 rounded-2xl border border-[#6f3032]/15 bg-white/55 px-4 py-3 text-sm text-[#6f3032]"
            ...
            <AnimatePresence>
              {submitError ? (
                <motion.p
                  className="mt-6 rounded-2xl border border-[#6f3032]/15 bg-white/55 px-4 py-3 text-sm text-[#6f3032]"
...
function TextField({
  ...
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-medium text-[#1f1a17]/72">{label}</span>
      <input
...
function SummaryPill({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-[1.15rem] border border-white/60 bg-white/42 px-5 py-4 shadow-sm">
      ...
      <p className="mt-1 font-display text-3xl text-[#6f3032]">{value}</p>
    </div>
  );
}
...
function GenderField({
  ...
  return (
    <fieldset>
      <legend className="mb-2 block text-sm font-medium text-[#1f1a17]/72">{genderLabel}</legend>
  ...
  return (
    <button
      ...
      className={`h-11 rounded-full border px-3 text-sm font-medium transition ${
        selected
          ? "border-[#6f3032] bg-[#6f3032] text-white shadow-[0_10px_28px_rgba(111,48,50,0.2)]"
          : "border-white/65 bg-white/42 text-[#1f1a17]/65 hover:bg-white/70"
      }`}
  ...
  return (
    <button
      ...
      className={`flex items-center justify-between rounded-[1.15rem] border px-5 py-4 transition ${
        selected
          ? "border-[#6f3032]/45 bg-[#6f3032]/10 text-[#6f3032] shadow-[0_14px_35px_rgba(111,48,50,0.12)]"
          : "border-white/65 bg-white/38 text-[#1f1a17]/70 hover:bg-white/62"
      }`}
      aria-pressed={selected}
    >
      <span className="font-medium">{label}</span>
      <span className={`grid h-6 w-6 place-items-center rounded-full border ${selected ? "border-[#6f3032] bg-[#6f3032] text-white" : "border-[#1f1a17]/18"}`}>
