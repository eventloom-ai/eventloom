  ...
  return (
    <form onSubmit={submit} className="mx-auto max-w-3xl rounded-2xl border border-black/[0.10] bg-white p-2 shadow-[0_26px_80px_rgba(54,42,30,0.10)]">
      ...
      <div className="flex items-center justify-between border-t border-black/[0.08] px-2 py-2">
        <span className="hidden text-[11px] text-[#86818a] sm:inline">No technical or design experience needed.</span>
        <button type="submit" disabled={!brief.trim()} className="ml-auto inline-flex min-h-11 items-center gap-2 rounded-xl bg-violet-600 px-4 py-2 text-[12px] font-semibold text-white transition hover:bg-violet-500 disabled:opacity-40"><ArrowUp className="size-3.5" /> Create free draft</button>
      </div>
    </form>
  );
}
