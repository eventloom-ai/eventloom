    ...
    <aside className="flex min-h-0 flex-col border-l border-white/10 bg-[#171717] text-white">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        {tab === "style" ? <div className="space-y-4"><div className="flex items-center gap-2"><Palette className="size-4 text-violet-300" /><p className="text-[12px] font-semibold">Style</p></div>{node ? <>
          ...
          <label className="grid gap-1.5 text-[10px] text-white/50">Texture<select value={node.style?.texture ?? "none"} disabled={disabled} onChange={(event) => style("texture", event.target.value)} className={selectClass}>{["none", "paper", "grain", "linen", "wash"].map((value) => <option key={value}>{value}</option>)}</select></label>
