...
export function StudioInspector({ eventId, node, config, disabled, onOperations, onEventPatch }: StudioInspectorProps) {
  ...
  return (
    <aside className="flex min-h-0 flex-col border-l border-white/10 bg-[#171717] text-white">
      <div className="grid grid-cols-3 border-b border-white/10 p-1.5">{(["content", "style", "details"] as const).map((item) => <button key={item} type="button" onClick={() => setTab(item)} className={`rounded-md px-2 py-2 text-[10px] font-medium capitalize ${tab === item ? "bg-white/10 text-white" : "text-white/40 hover:text-white"}`}>{item}</button>)}</div>
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        {tab === "content" ? <div><div className="mb-4 flex items-center gap-2"><Type className="size-4 text-violet-300" /><div><p className="text-[12px] font-semibold">{node?.label ?? node?.type ?? "Nothing selected"}</p><p className="text-[10px] text-white/35">{node ? node.id : "Select an element on the canvas."}</p></div></div>
        ...
        {tab === "details" ? <div className="space-y-4"><div className="flex items-center gap-2"><SlidersHorizontal className="size-4 text-violet-300" /><p className="text-[12px] font-semibold">Event details</p></div>{([
          ...
          <fieldset className="grid grid-cols-2 gap-2"><legend className="col-span-2 mb-1 text-[10px] text-white/50">RSVP fields</legend>{(["name", "attendance", "party_size", "guest_names", "email", "phone", "meal_preference", "note"] as const).map((field) => <label key={field} className="flex items-center gap-2 rounded-lg border border-white/10 px-2 py-2 text-[9px] text-white/55"><input type="checkbox" checked={config.rsvpFields.includes(field)} disabled={disabled || field === "name" || field === "attendance"} onChange={(event) => { const fields = event.target.checked ? [...config.rsvpFields, field] : config.rsvpFields.filter((value) => value !== field); onEventPatch({ rsvpFields: fields }, "Updated RSVP fields"); }} />{field.replaceAll("_", " ")}</label>)}</fieldset>
        </div> : null}
      </div>
    </aside>
  );
