...
const suggestions = ["Make the page feel more elegant", "Use warmer colors", "Make the first section stand out"];
...
export function StudioChat({ messages, value, selectedLabel, isRunning, activity, error, onChange, onSubmit, onStop, onClearSelection, onAttachment, attachmentName, uploadingAttachment, hideComposer }: StudioChatProps) {
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col overflow-hidden border-r border-white/10 bg-[#171717] text-white">
      <div className="border-b border-white/10 px-4 py-3"><div className="flex items-center gap-2 text-[12px] font-semibold"><WandSparkles className="size-4 text-violet-300" /> Ask Eventloom</div><p className="mt-1 text-[10px] leading-4 text-white/40">Describe a change in your own words, or click something on the preview.</p></div>
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4" aria-live="polite">
        {messages.length === 0 ? <div className="rounded-xl border border-dashed border-white/10 bg-white/[0.03] p-4"><p className="text-[12px] font-medium">Your page is ready to personalize.</p><p className="mt-1 text-[11px] leading-5 text-white/45">Ask for any change—colors, wording, photos, sections, or event details.</p></div> : null}
      ...
      {!hideComposer ? <div className="shrink-0 border-t border-white/10 p-3">
        ...
        <p className="mt-2 text-center text-[9px] text-white/25">Automatic changes use build credit. Side-panel edits are free.</p>
      </div> : null}
    </aside>
  );
}
