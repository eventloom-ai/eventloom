...
export function VisualStudio({ initialState, initialNotice }: VisualStudioProps) {
  ...
  return <main className="flex h-[100svh] min-h-[680px] flex-col overflow-hidden bg-[#111]">
    <StudioToolbar eventId={event.id} title={event.config.title} status={event.status} saveStatus={saveStatus} viewport={viewport} canUndo={canUndo} canRedo={redoStack.length > 0} onViewport={setViewport} onUndo={undo} onRedo={redo} onToggleHistory={() => setDrawer((current) => current === "history" ? null : "history")} onToggleCode={() => setDrawer((current) => current === "code" ? null : "code")} />
    <div className="relative min-h-0 flex-1">
      <div className="hidden h-full grid-cols-[320px_minmax(0,1fr)_300px] lg:grid">
        ...
        <div className="relative min-w-0"><button type="button" onClick={() => setOutlineOpen(true)} className="absolute left-3 top-3 z-30 inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-[#191919] px-2.5 py-1.5 text-[10px] text-white/55 shadow-lg hover:text-white"><Layers3 className="size-3.5" /> Outline</button><StudioCanvas document={revision.document} event={event} selectedNodeId={selectedNodeId} viewport={viewport} zoom={zoom} changedNodeIds={changedNodeIds} onSelect={(id) => setSelectedNodeId(id || null)} onTextCommit={(nodeId, content) => commitManual([{ op: "replace_text", nodeId, content }], undefined, "Edited text on canvas")} onZoom={setZoom} /></div>
      ...
      <div className="flex h-full flex-col lg:hidden">
        ...
        <nav className="grid h-12 shrink-0 grid-cols-3 border-t border-white/10 bg-[#171717] text-white">{([{ id: "chat", icon: MessageSquare }, { id: "canvas", icon: Layers3 }, { id: "design", icon: Palette }] as const).map(({ id, icon: Icon }) => <button key={id} type="button" onClick={() => setMobileTab(id)} className={`grid place-items-center text-[9px] capitalize ${mobileTab === id ? "text-violet-300" : "text-white/35"}`}><span className="grid place-items-center gap-0.5"><Icon className="size-4" />{id}</span></button>)}</nav>
      </div>
      ...
      {drawer ? <StudioDrawer mode={drawer} versions={versions} document={revision.document} currentVersionId={revision.id} disabled={Boolean(activeRunId)} onRestore={restore} onClose={() => setDrawer(null)} /> : null}
    </div>
  </main>;
}
