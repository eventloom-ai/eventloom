...
import { ensureDefaultOrganizationForUser, type OrganizationRecord } from "@/lib/organizations";
...
async function loadDashboardData(): Promise<DashboardData> {
  if (!isSupabaseConfigured()) {
    return { organization: null, events: demoEvents };
  }

  const user = await getServerUser();
  const client = serviceSupabase();
  if (!user || !client) {
    return { organization: null, events: [] };
  }
  ...
  return { organization, events: (data ?? []) as EventRecord[] };
}
...
export async function Dashboard() {
  const { organization, events } = await loadDashboardData();
