...
export default async function EventPrivacyPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <AppShell backHref={`/app/events/${eventId}/studio`} backLabel="Studio" width="narrow" title="Privacy and event timing" description="These details appear in the RSVP privacy context and determine the automatic deletion deadline. Publishing is blocked until they are complete."><EventPrivacySettings eventId={eventId} initial={{ controllerLegalName: settings.controller_legal_name ?? "", privacyContact: settings.privacy_contact ?? "", collectionPurpose: settings.collection_purpose ?? "", optionalFieldJustification: settings.optional_field_justification ?? "", endsAt: event.event_ends_at ?? event.ends_at ?? "", timezone: event.event_timezone ?? event.timezone ?? "America/Toronto" }} /></AppShell>;
}
