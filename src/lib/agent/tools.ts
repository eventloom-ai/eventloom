...
import { ensureDefaultOrganizationForUserId } from "@/lib/organizations";
...
function structuredRsvpDeadline(value?: string) {
...
export async function createEventRecord(input: {
  ...
  const organizationId =
    input.organizationId ?? (input.ownerId ? (await ensureDefaultOrganizationForUserId(input.ownerId))?.id : null);
  ...
  const { data, error } = await client
    ...
    .insert({
      ...
      event_type: input.config.eventType || null,
      rsvp_deadline_at: structuredRsvpDeadline(input.config.rsvpDeadline),
