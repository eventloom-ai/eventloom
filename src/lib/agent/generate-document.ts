  ...
  properties: {
    ...
    theme: {
      ...
      properties: {
        ...
        accent: { type: "string" },
        muted: { type: "string" },
        display: { type: "string", enum: ["editorial", "romantic", "modern", "playful", "bold", "vintage", "elegant", "condensed"] },
        body: { type: "string", enum: ["clean", "humanist", "geometric", "serif", "warm"] },
...
Structure comes first, before any styling decision:
- The page is a working flow, not a portfolio of decorative panels. Default shape: Opening (title, the couple/host, the one or two facts that are actually known) then Itinerary (only if the brief gives you a real schedule or sequence of moments — hour by hour, or event by event) then RSVP (always last, always the close of the page).
- Every section must be earned by real content — a fact, a schedule, the RSVP itself. Never create a section whose entire purpose is atmosphere (a quote on its own, a monogram on its own, a "note for the next page" interstitial). A quote, monogram, ornament, or overlay layer is an accent placed inside a real section, never a standalone section by itself.
- Let the brief's information density set the length. If most facts are still "to be announced," the honest page is short — as few as two sections: Opening and RSVP. Do not pad a thin brief out to five or six sections just to fill a template shape. A confident, short page beats a long one with nothing in most of it.
- The RSVP section is the destination, not an afterthought bolted to the bottom. Give it its own real background color (not just inherited page background), generous padding, and enough visual weight that it reads as a considered, high-quality panel — the place the whole page has been leading to — not a bare form dropped in.
...
- Atmosphere copy is allowed as an accent inside a real section. Invented facts are not. Never invent names, dates, times, venues, addresses, or URLs. Bind known facts to event.title, event.subtitle, event.date, event.venueName, event.venueAddress, or event.initials. If a fact is missing, say so in the bound value — do not add WHEN/WHERE/ADDRESS labels with empty values.
...
- Craft moves, use sparingly (one or two total, woven into a real section, never their own section): layer type over an image or color field with real depth using overlay rows — a big word behind a portrait, an accent panel behind offset text — not just for tint. Use divider dividerVariant "ornament" or "dot" instead of always defaulting to a plain line when a section wants a printed-invitation feel. Use text variant "quote" for a single atmosphere line that deserves emphasis, placed inside the section it supports — never invented, drawn from the brief's own mood. Bind a name block to event.initials for a monogram moment inside the opening or RSVP section when the brief calls for restraint. Use style.rotate and style.offset to break an otherwise even grid with an intentional, printed-collage tilt — never on the RSVP block or on long body copy.
...
function assembleDocument(raw: Record<string, unknown>, config: EventConfig, prompt: string): SiteDocument {
  ...
  const parsed = siteDocumentSchema.safeParse({
    ...
    theme: {
      ...
      typography: {
        display: ["editorial", "romantic", "modern", "playful", "bold", "vintage", "elegant", "condensed"].includes(String(theme.display)) ? theme.display : "editorial",
        body: ["clean", "humanist", "geometric", "serif", "warm"].includes(String(theme.body)) ? theme.body : "clean",
      },
